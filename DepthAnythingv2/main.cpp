...
int main(int argc, char** argv) {
    ...
    if (!options["model"].empty()) {
        ...
        // size_t total_params_count = depth_model.getTotalParameterCount();
        // num_params = total_params_count;
        cout << "Model successfully loaded." << endl << endl;
        // cout << "Model successfully loaded with " << num_params << " parameters." << endl << endl;
        model_loaded = true;
    }
    ...
    if (!options["camera"].empty()) {
        if (!model_loaded) {
            cout << "Model not loaded. Please specify a model path using --model." << endl;
            return -1;
        }
        ...
        while (true) {
            ...
            // Run depth prediction on the current frame
            ...
            // Display the result alongside the original frame
            cv::Mat result;
            cv::hconcat(frame, result_d, result);  // Concatenate horizotally

            int target_width = width/2; // Keep original width or reduce it further if desired
            int target_height = height/4; // Reduce height to half for a smaller window
            cv::resize(result, result, cv::Size(target_width, target_height));  // Resize to show both images
            imshow("Depth: Live Feed (Original -> Depth)", result);
